      ...
      <div class="register-boxTwo">
        ...
        <el-form :model="data.form" ref="formRef" :rules="data.rules">
          ...
          <el-form-item prop="password">
            <el-input :prefix-icon="Lock" size="large" v-model="data.form.password" placeholder="请输入新密码" show-password />
          </el-form-item>
          <el-form-item prop="confirmPassword">
            <el-input :prefix-icon="Lock" size="large" v-model="data.form.confirmPassword" placeholder="请确认新密码" show-password />
          </el-form-item>
          ...
          <el-form-item>
            <el-button size="large" type="primary" style="width: 100%; margin-top: 2%" @click="register">重置密码</el-button>
          </el-form-item>
        </el-form>
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <div>
            <a href="/ForgotUsername">忘记账号？</a>
          </div>
          <div>
...
const sendCode = async () => {
  ...
  // 表单验证
  formRef.value.validateField(['username', 'password', 'confirmPassword', 'captcha'], async (valid) => {
    if (valid) {
      try {
...
const register = () => {
  ...
  formRef.value.validate((valid) => {
    if (valid) {
      request.post('/ForgotPassword', data.form).then((res) => {
